...
public abstract class AbstractApplicationContext extends DefaultResourceLoader
	...
	public void refresh() throws BeansException, IllegalStateException {
		synchronized (startupShutdownMonitor) {
			...
			// 准备工作 包括设置启动时间，是否激活标志位
			// 初始化属性源 (property source)匹配
			prepareRefresh();

			// Tell the subclass to refresh the internal bean factory.
			// 返回一个factory，要对工厂进行初始化
			...
			try {
				...
				// 设置自定义的ProcessBeanFactory 和 spring 内部自定义的
	...
	protected void initMessageSource() {
		...
		if (beanFactory.containsLocalBean(MESSAGE_SOURCE_BEAN_NAME)) {
			...
			if (logger.isTraceEnabled()) {
				logger.trace("No '" + MESSAGE_SOURCE_BEAN_NAME + "' bean, using [" + messageSource + "]");
			}
		}
	}
	...
	protected void initApplicationEventMulticaster() {
		...
		if (beanFactory.containsLocalBean(APPLICATION_EVENT_MULTICASTER_BEAN_NAME)) {
			applicationEventMulticaster = beanFactory
			...
			if (logger.isTraceEnabled()) {
				logger.trace("No '" + APPLICATION_EVENT_MULTICASTER_BEAN_NAME + "' bean, using " + "["
						+ applicationEventMulticaster.getClass().getSimpleName() + "]");
			}
		}
	}
	...
	protected void initLifecycleProcessor() {
		...
		if (beanFactory.containsLocalBean(LIFECYCLE_PROCESSOR_BEAN_NAME)) {
			...
			if (logger.isTraceEnabled()) {
				logger.trace(
						"No '" + LIFECYCLE_PROCESSOR_BEAN_NAME + "' bean, using " + "[" + lifecycleProcessor.getClass()
								.getSimpleName() + "]");
			}
		}
	}
